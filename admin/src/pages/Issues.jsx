...
const Issues = () => {
  ...
  */
  ...
  // Actual API functionality (commented out)
  /*
                ...
                <tr key={issue._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {typeof issue.location === 'object' && Object.keys(issue.location).length === 0 
                        ? 'No Location' 
                        : (typeof issue.location === 'string' ? issue.location : 'No Location')}
                    </div>
                    <div className="text-sm text-gray-500">{issue.address || 'No Address'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(issue.status || 'pending')}`}>
                      ...
                      <div className="flex-shrink-0 h-8 w-8">
                        <div className="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center">
                          <span className="text-sm font-medium text-gray-500">
                            {typeof issue.reportedBy === 'string' ? '#' : issue.reportedBy.fullName?.[0] || '?'}
                          </span>
                        </div>
                      </div>
                      <div className="ml-4">
                        <div className="text-sm font-medium text-gray-900">
                          {typeof issue.reportedBy === 'string' ? `User ${issue.reportedBy.slice(-4)}` : issue.reportedBy.fullName}
                        </div>
                        <div className="text-sm text-gray-500">
                          {typeof issue.reportedBy === 'string' ? 'No email available' : issue.reportedBy.email}
                        </div>
                      </div>
                    </div>
                  </td>
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl max-w-2xl w-full">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="mt-2 flex gap-2">
                  {['pending', 'in-progress', 'resolved', 'rejected'].map((status) => (
                    <button
                      key={status}
                      onClick={() => handleUpdateStatus(selectedIssue._id, status)}
                ...
                <div>
                  ...
                  <dl className="mt-2 space-y-2">
                    <div>
                      ...
                      <dd className="text-sm text-gray-900">{selectedIssue.title}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Description</dt>
                      <dd className="text-sm text-gray-900">{selectedIssue.description}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Priority</dt>
                      <dd className={`text-sm font-medium ${getPriorityColor(selectedIssue.priority)}`}>
                ...
                <div>
                  ...
                  <dl className="mt-2 space-y-2">
                    <div>
                      ...
                      <dd className="text-sm text-gray-900">{selectedIssue.location}</dd>
                    </div>
                    <div>
                      <dt className="text-sm font-medium text-gray-500">Address</dt>
                      <dd className="text-sm text-gray-900">{selectedIssue.address}</dd>
                    </div>
                  </dl>
                </div>
              </div>
              ...
              {selectedIssue.images && selectedIssue.images.length > 0 && (
                <div>
                  <h3 className="text-lg font-medium text-gray-900">Issue Images</h3>
